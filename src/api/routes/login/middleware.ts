...
const mainHost = new URL(config[process.env.NODE_ENV === "production" ? "gst5" : "api"].publicUrl).host;
...
export async function redirectToMainDomain (ctx: ParameterizedContext, next: Next): Promise<void> {
    ...
    if(ctx.host !== mainHost) {
        ctx.redirect(`${config[process.env.NODE_ENV === "production" ? "gst5" : "api"].publicUrl}${ctx.originalUrl}`);
        return;
    }
    await next();
}